...
// 	result, err := db.QueryDatabasePartition(axon_types.AXON_TABLE, fmt.Sprintf("FOLDER#%s", f.Session.SessionData.User.Email), &indexName)
// 	if err != nil {
// 		return nil, errors.New("could not fetch folders - " + err.Error())
// 	}
...
// 			result, _ := db.QueryDatabasePartition(axon_types.AXON_TABLE, fmt.Sprintf("NOTE#%s#%s", f.Session.SessionData.User.Email, item.FolderID), &indexName)
