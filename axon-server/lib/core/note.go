...
// 	nodeResult, err := db.QueryDatabasePartition(axon_types.AXON_TABLE, fmt.Sprintf("NODE#%s#%s#%s", n.Session.SessionData.User.Email, folder_id, note_id), nil)

// 	if err != nil {
// 		return nil, errors.New("could not fetch node details - " + err.Error())
// 	}

// 	edgeResult, err := db.QueryDatabasePartition(axon_types.AXON_TABLE, fmt.Sprintf("EDGE#%s#%s#%s", n.Session.SessionData.User.Email, folder_id, note_id), nil)
...
// 		return nil, errors.New("could not fetch note detail - " + err.Error())
// 	}
// 	var indexName string = "date_createdIndex"
// 	// Fetch the Note
// 	notesResult, err := db.QueryDatabasePartition(axon_types.AXON_TABLE, fmt.Sprintf("NOTE#%s#%s", n.Session.SessionData.User.Email, folder_id), &indexName)
...
// func (n *Note) CreateNote(a *axon_types.AxonContext, note_name string, description string, folder_id string) (*axon_types.Note, error) {
...
// func (n *Note) UpdateNote(a *axon_types.AxonContext, note_name *string, description *string, folder_id string, note_id string) (*string, error) {
...
// 	err = db.UpdateRecord(axon_types.AXON_TABLE, fmt.Sprintf("NOTE#%s#%s", n.Session.SessionData.User.Email, folder_id), note_id, attributes)
